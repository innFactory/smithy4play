...
abstract class Smithy4PlayMiddleware extends Logging {
  ...
  /**
   * Override to skip middleware for certain requests.
   * Default: apply middleware to all requests.
   */
  def skipMiddleware(r: RoutingContext): Boolean = false

  /**
   * Called when middleware is skipped. Override for custom telemetry.
   * @return The value passed in (for chaining)
   */
   ...
   * 
   * @param r The routing context for the current request
   * @param next The next handler in the chain (call this to continue processing)
  ...
  /**
   * Construct a ContextRoute from this middleware.
   * Used internally for Kleisli-based composition.
   */
