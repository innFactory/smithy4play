...
/** Utility object for creating Blobs lazily from Play's RawBuffer.
  *
  * Since Smithy4s Blob is sealed, we can't extend it directly. Instead, this provides factory methods that defer
  * materialization until the Blob is actually needed.
  */
object LazyBlob {

  /** Create a Blob from a RawBuffer with lazy materialization.
    *
    * This avoids unnecessary memory copies for requests where the body is not needed (e.g., validation failures, GET
    * requests).
    *
    * Note: The actual bytes are read when this method is called, but the method itself should be called lazily (via
    * lazy val).
    */
...
/** Wrapper that provides lazy Blob creation.
  *
  * Use this when you want to defer body parsing until it's actually needed. The Blob is created on first access and
...
final class LazyBlobHolder(rawBuffer: RawBuffer) {
  ...
  /** Lazily materialized Blob. The RawBuffer is only read when this is first accessed.
    */
