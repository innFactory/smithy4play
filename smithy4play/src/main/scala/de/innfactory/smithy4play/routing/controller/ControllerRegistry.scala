...
/** Registry for auto-routable controllers with caching support.
  *
  * This object caches classpath scan results to avoid expensive re-scanning on every router instantiation. The cache is
...
object ControllerRegistry {
  ...
  /** Get cached controller classes for the given package, or scan if not cached.
    *
    * @param pkg
    *   The package to scan for controllers implementing [[AutoRoutableController]]
    * @return
    *   Sequence of controller classes found in the package
    */
  def getControllers(pkg: String): Seq[Class[?]] =
    cache.computeIfAbsent(pkg, scanPackage)

  /** Explicitly register controller classes for a package. This bypasses classpath scanning entirely.
    *
    * @param pkg
    *   The package identifier (used as cache key)
    * @param controllers
    *   The controller classes to register
    */
  ...
  private def scanPackage(pkg: String): Seq[Class[?]] = {
    ...
    try {
      ...
      controllers
    } finally classGraphScanner.close()
  }
}
