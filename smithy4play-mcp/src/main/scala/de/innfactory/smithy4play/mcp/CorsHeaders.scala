...
/** CORS headers configuration for MCP endpoints.
  *
  * SECURITY NOTE: The default configuration allows all origins ("*") which is suitable for development but should be
  * restricted in production. Override `allowedOrigins` or use `withAllowedOrigins` to specify allowed origins for your
  * deployment.
  */
object CorsHeaders {
  ...
  /** Apply CORS headers to a result, allowing all origins. Use `applyCorsHeaders(result, request, allowedOrigins)` for
    * production.
    */
  ...
  /** Apply CORS headers to a result with configurable allowed origins.
    *
    * @param result
    *   The result to add headers to
    * @param request
    *   The request (used to extract the Origin header)
    * @param allowedOrigins
    *   Set of allowed origins, or None to allow all ("*")
    */
