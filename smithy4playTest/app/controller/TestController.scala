...
class TestController @Inject() (implicit
  ...
  override def testWithOutput(pathParam: String, testQuery: String, testHeader: String, body: TestRequestBody): ContextRoute[TestWithOutputResponse] =
    Kleisli { rc =>
      EitherT.rightT[Future, ContextRouteError](TestWithOutputResponse(TestResponseBody(testHeader, pathParam, testQuery, body.message)))
    }
