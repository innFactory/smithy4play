...
class BasicSmithy4PlaySimulation extends Simulation {
  private val baseUrl = System.getProperty("baseUrl", "http://localhost:9000")
  ...
  private val getHealth = scenario("GET health")
    .exec(
      http("health").get("/health").check(status.is(200))
    )

  private val postJson = scenario("POST testWithOutput")
    .exec(
      http("testWithOutput")
  ...
  setUp(
    getHealth.inject(rampUsers(totalUser) during (ramp))
      .andThen(postJson.inject(rampUsers(totalUser) during (ramp)))
      .andThen(postJson.inject(constantUsersPerSec(math.max(1, totalUser / 2.0)) during (steady)))
